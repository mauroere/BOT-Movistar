...
class CDUSelectorManager {
  ...
  initAfterDOM() {
    ...
    try {
      ...
      if (!elementsFound) {
        console.error(
          "❌ CDUSelectorManager: No se pudieron encontrar elementos DOM requeridos"
        );
        return;
      }
      ...
      if (!listenersSetup) {
        console.error(
          "❌ CDUSelectorManager: No se pudieron configurar event listeners"
        );
        return;
      }
      ...
      if (typeof window !== "undefined") {
        ...
        console.log(
          "   Para hacer test manual: window.debugCDUSelector.testToggle()"
        );
        console.log(
          "   Para actualizar CDUs: window.debugCDUSelector.updateCDUList()"
        );
        console.log(
          "   Para revisar flujos: window.debugCDUSelector.bot.botFlows"
        );
      }
    } catch (error) {
  ...
  bindElements() {
    ...
    requiredElements.forEach((key) => {
      ...
      console.log(
        `  ${key}: ${found ? "✅" : "❌"} ${
          found ? `(classes: ${classes})` : "NO ENCONTRADO"
        }`
      );
    ...
    if (this.elements.toggleBtn) {
      ...
      console.log(
        "  - Parent:",
        this.elements.toggleBtn.parentElement?.className
      );
    ...
    if (allElementsFound) {
      console.log(
        "✅ CDUSelector: Todos los elementos DOM vinculados correctamente"
      );
    } else {
  ...
  createEnhancedSelector() {
    ...
    // Crear sección de favoritos - COMPLETAMENTE DESHABILITADO
  ...
  createEnhancedHeader() {
    const existingHeader = this.elements.selector.querySelector(
      ".cdu-selector-header"
    );
  ...
  // FUNCIÓN DESHABILITADA - createFavoritesSection()
  /*
  ...
  */
  ...
  setupBasicEventListeners() {
    console.log(
      "🔗 CDUSelectorManager: Configurando event listeners básicos..."
    );
    ...
    if (this.elements.toggleBtn) {
      ...
      console.error(
        "❌ Toggle button no encontrado - no se pueden configurar listeners"
      );
      return false;
    }
  ...
  basicToggle() {
    ...
    if (this.elements.toggleBtn) {
      ...
      if (icon) {
        const newIconClass = isCollapsed
          ? "fas fa-chevron-up"
          : "fas fa-chevron-down";
  ...
  updateSortIcon() {
    ...
    this.elements.sortBtn.title =
      tooltips[this.currentSort] || "Cambiar ordenamiento";
  }
  ...
  updateStatsIcon(isCollapsed) {
    if (this.elements.statsToggle) {
      ...
      this.elements.statsToggle.title = isCollapsed
  ...
  getCDUList() {
    ...
    const availableCDUs = developedCDUs.filter(
      (cduId) => this.bot.botFlows[cduId]
    );
  ...
  sortByRecent(cdus) {
    ...
    const recentMap = new Map(
      ranking.map((item) => [item.id, new Date(item.lastUsed || 0)])
    );
  ...
  renderCDUGrid(cdus) {
    ...
    this.elements.grid.innerHTML = cdus
      .map((cdu) => {
        const usageCount = this.statsManager
          ? this.statsManager.getUsageCount(cdu.id)
          : 0;
        const isFavorite = this.statsManager
          ? this.statsManager.isFavorite(cdu.id)
          : false;
        ...
        <button class="cdu-btn ${isFavorite ? "favorite" : ""}" 
          ...
          <div class="cdu-btn-content">
            ...
            ${
              usageCount > 0
                ? `<span class="cdu-usage-badge">${usageCount}</span>`
                : ""
            }
          </div>
          ${
            isFavorite
              ? '<div class="favorite-indicator"><i class="fas fa-star"></i><span class="usage-count">' +
                usageCount +
                "</span></div>"
              : ""
          }
